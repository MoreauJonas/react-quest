...
window.resizeToFit = (function makeResizeToFit() {
    ...
    function setProp(selector, properties) {
      if (sel2Id.has(selector)) {
        ...
        properties.forEach(function eachProp(prop) {
          ...
          styleSheet.cssRules.item(idx).style.setProperty(propName, propValue);
        });
      } else {
        // Create new rule
        let propString = "";
  ...
  function calculateFontSize(elObj, selector) {
    ...
    const currentFontSize =
      collection.get(selector).get("resizedFontSize") || elObj.originalFontSize;
  ...
  function resize() {
    ...
    collection.forEach(function eachSelectorC(selector) {
      ...
      cssHandler.setProp(selector.get("name"), [
        "overflow: hidden",
        "display: block",
        "font-size: " + selector.get("originalFontSize") + "px",
      ]);
    ...
    collection.forEach(function eachSelectorP(selector) {
      if (
        selector.get("originalFontSize") !== selector.get("resizedFontSize")
      ) {
        cssHandler.setProp(selector.get("name"), [
  ...
  function init(selectors) {
    ...
    selectors.forEach(function eachSelector(selector) {
      ...
      nodeList.forEach(function eachElement(element) {
        ...
        const originalElementFontSize = parseFloat(computedStyles.fontSize);
